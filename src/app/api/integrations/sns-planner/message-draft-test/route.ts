...
const messageDraftTestPayload = {
  ...
  channel: "line",
  purpose: "post_session_followup",
  audienceSegment: "repeat_customers",
...
export async function POST(request: NextRequest) {
  ...
  const inboundSourceApp =
    request.headers.get("X-Source-App") ?? messageDraftTestPayload.sourceApp;
  ...
  try {
    ...
    try {
      responseBody = JSON.parse(responseText) as SnsPlannerMessageDraftResponse;
    } catch {
    ...
    const recordedMessageDraft = {
      messageDraftId:
        asString(maybeDraft.messageDraftId) ??
        asString(maybeDraft.draftId) ??
        asString(maybeDraft.id),
      workspaceId:
        asString(maybeDraft.workspaceId) ?? messageDraftTestPayload.workspaceId,
      status:
        asString(maybeDraft.messageDraftStatus) ?? asString(maybeDraft.status),
    ...
    const inboundApiLog = {
      ...
      targetApp: messageDraftTestPayload.sourceApp,
      operation: "GrowthEngine.MessageDraftTest",
      endpoint: "POST /api/integrations/sns-planner/message-draft-test",
      workspaceId: messageDraftTestPayload.workspaceId,
      userId: messageDraftTestPayload.userId,
    ...
    const outboundApiLog = {
      ...
      traceId,
      correlationId,
      sourceApp: messageDraftTestPayload.sourceApp,
