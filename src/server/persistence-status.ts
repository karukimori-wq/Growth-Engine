...
export async function getPersistenceStatus() {
  ...
  const databaseBackedPersistenceReady = d1Health?.databaseBackedPersistenceReady
    ?? postgresHealth?.databaseBackedPersistenceReady
    ?? false;
  const status = databaseBackedPersistenceReady
    ? "success"
    : (d1Health?.status ?? postgresHealth?.status ?? "warning");
  ...
  return {
    ...
    requiredEnvCandidates: repositoryDriver === "postgres"
      ? preflight.env.candidates.map((candidate) => candidate.name)
      : [],
    preflight: {
    ...
    nextAction: databaseBackedPersistenceReady
      ...
      : "Configure a reachable database-backed Growth Repository driver.",
    nextActions: databaseBackedPersistenceReady
      ? ["Run POST /api/persistence/roundtrip from an owner session."]
      : ["Configure GROWTH_REPOSITORY_DRIVER=d1 with the DB binding, or restore a reachable Postgres runtime."],
    issues: databaseBackedPersistenceReady
      ? []
      : [d1Health?.issue ?? postgresHealth?.issue ?? preflight.issues[0]].filter(Boolean),
